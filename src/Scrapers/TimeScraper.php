...
class TimeScraper extends BaseScraper implements TimeScraperInterface
{
    ...
    public function scrape(string|int $raceCode, CarbonInterface|string|null $date = null): array
    {
        ...
        foreach (range(1, 6) as $boatNumber) {
            ...
            $racerExhibitionTime = Normalizer::normalize($times['.col5'][$boatNumber] ?? 0.0);
            $racerHalfLapTime = Normalizer::normalize($times['.col6'][$boatNumber] ?? 0.0);
            $racerTurnTime = Normalizer::normalize($times['.col7'][$boatNumber] ?? 0.0);
            $racerStraightTime = Normalizer::normalize($times['.col8'][$boatNumber - 1] ?? 0.0);
            ...
            $response['boat_number_' . $boatNumber . '_racer_half_lap_time'] = $racerHalfLapTime;
            ...
            $response['boat_number' . $boatNumber . '_racer_half_lap_time'] = $racerHalfLapTime;
